...
public class BudgetAnalyzerWithDeepSeek {
    ...
    private static final String CSV_FILE_PATH = "E:\\untitled4\\src\\main\\java\\org\\example\\input.csv"; // 替换为你的CSV文件路径

    public static void main(String[] args) {
        BudgetAnalyzerWithDeepSeek analyzer = new BudgetAnalyzerWithDeepSeek();
        try {
            String analysisResult = analyzer.analyzeBudgetWithDeepSeek(CSV_FILE_PATH);
            System.out.println(analysisResult); // 打印结果（可选）
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
